...
from enrichment_agent.tools import scrape_website, search, call_web_agent
...
async def call_agent_model(
    ...
    # Use the updated MAIN_PROMPT from prompts.py
    p = configuration.prompt.format(
        info=json.dumps(state.extraction_schema, indent=2), topic=state.topic, websites=state.websites
    )
    ...
    model = raw_model.bind_tools([call_web_agent, info_tool], tool_choice="any")
...
workflow.add_node("tools", ToolNode([call_web_agent])) #[search, scrape_website, call_web_agent]))
...
async def main():
    ...
    # Updated extraction schema to emphasize car insurance quotes
    extraction_schema = {
        "type": "object",
        "properties": {
            "quotes": {
    ...
    # Updated topic focusing on insurance quotes
    topic = f"Get online car insurance quotes for a 2020 Toyota Camry in San Francisco, CA. The full details are: Vehicle: Toyota Camry, 2020, VIN optional (if asked) Driver: 30 years old, no prior accidents, single, good credit score Location: San Francisco, CA, ZIP code 94103 Coverage: Standard liability coverage with $500 deductible. Use the following websites to get quotes: {INSURANCE_WEBSITES}"
